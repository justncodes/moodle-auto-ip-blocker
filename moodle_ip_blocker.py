...
if __name__ == "__main__":
    ...
    try:
        ...
        for ip, count in ip_failures.items():
            if count >= failure_threshold:
                if ip not in blocked_ips_this_run:
                    ...
                    if call_moodle_cli(config, ip): blocked_ips_this_run.add(ip)
                    else: logger.error(f"Skipping further actions for {ip} in this run due to CLI execution failure.")
                else: logger.debug(f"IP {ip} already processed for blocking in this run.")
        ...
        if 'new_max_id' in locals() and 'last_processed_id' in locals() and new_max_id > last_processed_id:
